...
function addOutputMessage (messageType, message) {
  store && store.dispatch({ type: 'ADD_OUTPUT_MESSAGE', messageType, message })
}
...
function run (file, method) {
  ...
  if (fileOverride) {
    const trimmedFile = fileOverride.trim()
...
export function stop () {
  ...
  store && store.dispatch({ type: 'STOP' })
}
...
export function get (goget, goconfig1) {
  goconfig = goconfig1
